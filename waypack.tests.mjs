...
{
  const src = html;
  check('a route is only re-asked when you have really left it, not on every GPS fix', /offRouteM: 60/.test(src) && /moveM: 150/.test(src) && /off > ROUTE_LIMITS.offRouteM && moved > ROUTE_LIMITS.moveM/.test(src));
  check('requests are spaced and never run in parallel', /minGapMs: 20000/.test(src) && /if \(ROUTE.inFlight\) return;/.test(src) && /Date.now\(\) - ROUTE.at < ROUTE_LIMITS.minGapMs/.test(src));
