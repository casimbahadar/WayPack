...
// ---------- 49. v56: names, routing honesty, migration, wanderers, free rest ----------
{
  ...
  { const src = html; check('each travel mode has its own router; walking no longer uses the car-only demo server', /routed-foot\/route\/v1\/foot/.test(src) && /routed-bike\/route\/v1\/bike/.test(src) && /routed-car\/route\/v1\/driving/.test(src) && !/router\.project-osrm\.org/.test(src) && /maxKmh: 8/.test(src)); }
